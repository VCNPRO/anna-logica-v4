...
try {
  ...
  transcribe = new AWS.TranscribeService({ region: process.env.AWS_REGION || 'us-east-1' });
} catch (error) {
  console.log('AWS SDK not available, using fallback mode');
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (event.httpMethod === 'POST') {
      ...
      try {
        // Step 1: Upload file to S3 (if AWS SDK available)
        if (s3) {
          ...
          console.log('⚠️ S3 not available, proceeding with intelligent processing');
        }

        // Step 2: Start AWS Transcribe job (if available)
        if (s3 && transcribe) {
          ...
          // AWS services not available, proceed with intelligent processing
          console.log('⚠️ AWS Transcribe not available, using intelligent fallback');
          throw new Error('AWS services not configured for real transcription');
        }
