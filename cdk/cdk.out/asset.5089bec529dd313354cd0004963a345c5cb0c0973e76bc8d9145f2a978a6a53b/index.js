...
exports.handler = async (event) => {
  ...
  try {
    ...
    // If no filePath, treat as health check
    if (!filePath || Object.keys(requestBody).length === 0) {
      console.log('💊 Health check via POST (no filePath)');
      return {
        statusCode: 200,
        headers: {
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify({
          ...
          provider: 'AWS Lambda + FFmpeg + Gemini',
          timestamp: new Date().toISOString(),
          message: 'Health check via POST endpoint'
        })
      };
    }
